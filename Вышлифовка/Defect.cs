...
namespace Вышлифовка
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            double shiftDiscontity = 20;
            double perimeter = Math.Round(Convert.ToDouble(diameterTextBox.Text) * Math.PI);
            List<Defect> result = new List<Defect>();
            for (int i = 0; i < defectsFor.Count; i++)
            {
                if (defectsFor[i].isDiscontinuity)//если дефект это несплошность, просто добавляем к ней по 20 мм в начале и в конце
                {
                    Defect def1 = new Defect();
                    Defect def2 = new Defect();
                    def1.start = defectsFor[i].start - shiftDiscontity;
                    def1.defectLength = defectsFor[i].defectLength +2 * shiftDiscontity;
