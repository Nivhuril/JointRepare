...
namespace Вышлифовка
{
    ...
    public partial class Form1 : Form
    {
        private Report get_report_from_file(string fileName)//метод для чтения протокола из файла
        {
            ...
            report.note = Convert.ToString(ObjWorkSheetReport.Cells[2, 1].Text);
        ...
        private Report notNullItems(Report report)
            {
            if (String.IsNullOrWhiteSpace(Convert.ToString(report.thekness)))
            {
                report.thekness = "0";
            }
            if (String.IsNullOrWhiteSpace(Convert.ToString(report.protocolNumber)))
            {
                report.thekness = "0";
            }
            if (String.IsNullOrWhiteSpace(Convert.ToString(report.objectName)))
            {
                report.objectName = "0";
            }
            if (String.IsNullOrWhiteSpace(Convert.ToString(report.date)))
            {
                report.date = "0";
            }
            if (String.IsNullOrWhiteSpace(Convert.ToString(report.jointName)))
            {
                report.jointName = "0";
            }
            if (String.IsNullOrWhiteSpace(Convert.ToString(report.productName)))
            {
                report.productName = "0";
            }
            if (String.IsNullOrWhiteSpace(Convert.ToString(report.perimeter)))
            {
                report.perimeter = "0";
            }
            if (String.IsNullOrWhiteSpace(Convert.ToString(report.pipeDiameter)))
            {
                report.perimeter = "0";
            }
